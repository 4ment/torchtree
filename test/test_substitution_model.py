...
def test_general_symmetric():
    ...
    subst_model = GeneralSymmetricSubstitutionModel('gen', Parameter('mapping', mapping), Parameter('rates', rates),
                                                    Parameter('pi', pi))
...
def test_GTR_batch():
    ...
    P_expected = torch.tensor(np.array(
        [
            [[[0.93717830, 0.009506685, 0.047505899, 0.005809115],
              [0.02640748, 0.894078744, 0.006448058, 0.073065722],
              [0.16158572, 0.007895626, 0.820605951, 0.009912704],
...
def test_HKY_batch(hky_fixture):
    kappa, pi, hky_P_expected, branch_lengths = hky_fixture
    kappa = Parameter(None, torch.cat((torch.tensor([2.], dtype=kappa.dtype), kappa), 0).unsqueeze(-1))
    frequencies = Parameter(None, torch.cat((torch.full_like(pi, 0.25), pi), 0).reshape((2, 4)))
    hky = HKY('hky', kappa, frequencies)
    probs = hky.p_t(torch.tensor([[5, 10., 20.0], [0.5, 0.1, 0.001]], dtype=kappa.dtype))
...
def test_JC69_batch():
    jc = JC69('jc')
    probs = jc.p_t(torch.tensor([[[0.5, 1., 2.0]], [[5, 10., 20.0]]]))
