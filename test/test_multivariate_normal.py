...
@pytest.mark.parametrize("parameterization,param", [("scale_tril", [[1.0, 0.0], [2.0, 3.0]]),
...
def test_parameterization(parameterization, param):
    ...
    dist_model = MultivariateNormal(None, Parameter(None, x), Parameter(None, loc), **kwargs)

    dist = torch.distributions.multivariate_normal.MultivariateNormal(loc, **{parameterization: param_tensor})
